...
;(function($){
	$.fn.skygqCheckAjaxForm = function(settings){
		...
		var	msg = "",
			...
			showRight = function(fieldObj,filedName,SuccessInfo){
				...
				_trackFormMsg = SuccessInfo + "::passed";//给变量赋值；
			},
			//focus时提示
			showExp = function(obj){
				...
				if (i.focusMsg){
					...
					_trackFormMsg = i.focusMsg + "::failed";//给变量赋值；
				}
			},
			//匹配对比值的提示名
	...
	$.skygqCheckAjaxForm.rule	= {
		...
		"username" 	: [/^[\u4E00-\u9FA5\w]{2,16}$/,"用户名不合法"],
		"password" 	: [/^(\w){4,20}$/,"只能为数字和英文及下划线的组合，4-20个字符"],
		"safe" 		: [/>|<|,|\[|\]|\{|\}|\?|\/|\+|=|\||\'|\\|\"|:|;|\~|\!|\@|\#|\*|\$|\%|\^|\&|\(|\)|`/i,"不能有特殊字符"],
		...
		"tel"       : [/^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})+(\-[0-9]{1,4})?$|(^(13[0-9]|15[0|3|6|7|8|9]|18[8|9])\d{8}$)/,"格式不正确"],
